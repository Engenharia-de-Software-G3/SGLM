...
export async function getLocacoesFunction(): Promise<ListManyLocacoes> {
  ...
  if (Array.isArray((raw as unknown as Record<string, unknown>)?.locacoes)) {
    locacoesRaw = (raw as unknown as Record<string, unknown>).locacoes as LocacaoInterface[];
    ultimoDoc = ((raw as unknown as Record<string, unknown>).ultimoDoc as string) ?? null;
...
export async function createLocacaoFunction(
  payload: CreateLocacaoInterface,
): Promise<LocacaoInterface> {
...
export async function updateLocacaoFunction(
  id: string,
  payload: UpdateLocacaoInterface,
): Promise<LocacaoInterface> {
