...
export const vehicleFormSchema = z.object({
  ...
  placa: z.string().nonempty('Placa é obrigatória').regex(
    /^[A-Z]{3}[0-9][0-9A-Z][0-9]{2}$/,
    'Placa deve estar no formato Mercosul (ABC1D23) ou antigo (ABC1234)',
  ),
  ano: z.string().nonempty('Ano é obrigatório').regex(
    /^\d{4}\/\d{4}$/,
    'Ano deve estar no formato AAAA/YYYY'
  ),
  ...
  quilometragemCompra: z
    ...
    .regex(/^\d+$/, 'Quilometragem deve ser numérica'),
  dataCompra: z.string().nonempty('Data da compra é obrigatória').regex(
    /^\d{2}\/\d{2}\/\d{4}$/,
  ...
  status: z.enum(['disponivel', 'locado', 'manutenção']).default('Disponível'),
});